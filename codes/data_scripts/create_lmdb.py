...
def vimeo90k(mode):
    ...
    BATCH = 5000  # After BATCH images, lmdb commits, if read_all_imgs = False
    if mode == 'GT':
        img_folder = '../../datasets/Youku/GT_2P'
        lmdb_save_path = '../../datasets/Youku/Youku_train_GT_2P.lmdb'
        txt_file = '../../datasets/Youku/youku_trainlist.txt'
        H_dst, W_dst = 256, 448
    elif mode == 'LR':
        img_folder = '../../datasets/Youku/crf_2428'
        lmdb_save_path = '../../datasets/Youku/Youku_train_DI.lmdb'
        txt_file = '../../datasets/Youku/youku_trainlist.txt'
        H_dst, W_dst = 256, 448
    elif mode == 'flow':
    ...
    print('data size per image is: ', data_size_per_img)
    data_size = data_size_per_img * len(all_img_list)
    env = lmdb.open(lmdb_save_path, map_size=data_size * 2) #10
    txn = env.begin(write=True)
    pbar = util.ProgressBar(len(all_img_list))
    for idx, (path, key) in enumerate(zip(all_img_list, keys)):
        pbar.update('Write {}'.format(key))
    ...
    if mode == 'GT':
        ...
        meta_info['name'] = 'Vimeo90K_train_DI'
    elif mode == 'flow':
