...
class Vimeo90KDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        for i in range(opt['N_frames']):
            ...
            self.LQ_frames_list.append(i + 1)
    ...
    def __getitem__(self, index):
        ...
        img_GT_l = []
        if self.data_type == 'mc':
            img_GT = self._read_img_mc(osp.join(self.GT_root, name_a, name_b, '3.png'))####
            img_GT = img_GT.astype(np.float32) / 255.
        elif self.data_type == 'lmdb':
            img_GT1 = util.read_img(self.GT_env, key + '_1', (3, 256, 448))
            img_GT2 = util.read_img(self.GT_env, key + '_2', (3, 256, 448))
            img_GT_l.append(img_GT1)
            img_GT_l.append(img_GT2)
        else:
            img_GT = util.read_img(None, osp.join(self.GT_root, name_a, name_b, 'im3.png')) ###im4
        ...
        for v in self.LQ_frames_list:
            if self.data_type == 'mc':
                img_LQ = self._read_img_mc(
                    osp.join(self.LQ_root, name_a, name_b, '{}.png'.format(2*v-1)))###v
                img_LQ = img_LQ.astype(np.float32) / 255.
            elif self.data_type == 'lmdb':
                img_LQ = util.read_img(self.LQ_env, key + '_{}'.format(v), LQ_size_tuple) ###v
            else:
                img_LQ = util.read_img(None,
                                       osp.join(self.LQ_root, name_a, name_b, 'im{}.png'.format(2*v-1)))
        ...
        if self.opt['phase'] == 'train':
            ...
            if self.LR_input:
                ...
                rnd_h = random.randrange(0, max(0, H - LQ_size), 2)
                rnd_w = random.randrange(0, max(0, W - LQ_size), 2)
                ...
                rnd_h = random.randrange(0, max(0, H - GT_size), 2)
                rnd_w = random.randrange(0, max(0, W - GT_size), 2)
